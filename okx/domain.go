...
// okx:// URIs by routing to the operations Register installs. The same
// Domain also builds the standalone okx binary (see cli.NewApp), so the
...
// Domain is the okx driver. It carries no state; the per-run client is
...
func (Domain) Register(app *kit.App) {
	...
	// Resolver op: one record per id, the home of `okx page` and
	// `ant get okx://page/<id>`.
	kit.Handle(app, kit.OpMeta{Name: "page", Group: "read", Single: true,
		Summary: "Fetch a page by path or URL", URIType: "page", Resolver: true,
		Args: []kit.Arg{{Name: "ref", Help: "page path or URL"}}}, getPage)

	// List op: members of a page, the home of `okx links` and `ant ls`.
	// It emits page stubs, so every listed member is itself an addressable
	// okx://page/ URI a host can follow.
	kit.Handle(app, kit.OpMeta{Name: "links", Group: "read", List: true,
		Summary: "List the pages a page links to", URIType: "page",
		Args: []kit.Arg{{Name: "ref", Help: "page path or URL"}}}, listLinks)

	// Search op: a free-text query, the home of `okx search` and the
	...
	// stubs, so a host can follow any hit to its own okx://page/ URI.
...
// Classify turns any accepted input — a bare path or a full okx.com URL —
// into the canonical (type, id), so `ant resolve` and `ant url` touch no network.
func (Domain) Classify(input string) (uriType, id string, err error) {
	id = pagePath(input)
	if id == "" {
		return "", "", errs.Usage("unrecognized okx reference: %q", input)
	}
	return "page", id, nil
}

...
describes what users get. Pick a format with `-o`, or let okx choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
okx <command> -o table     # a rounded, color-aware grid for reading
okx <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
okx <command> -o list      # one record per section, easy on the eyes
okx <command> -o jsonl     # one JSON object per line, for piping
okx <command> -o json      # a single JSON array
okx <command> -o csv       # spreadsheet friendly
okx <command> -o tsv       # tab-separated
okx <command> -o url       # just the URL column
okx <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
okx <command> --fields id,title,url
```
...
```bash
okx <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
okx <command>            # a table, because this is a terminal
okx <command> | wc -l    # JSONL, because this is a pipe
```
